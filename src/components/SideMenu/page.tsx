...
export default function SideMenu() {
  ...
  const loadProfile = async (index: number) => {
    ...
    const profile = profiles[index] as { name: string; mods: string[] };
  ...
  const deleteProfile = async (index: number) => {
    ...
    const profile = profiles[index] as { name: string };
  ...
  const startRename = (index: number) => {
    ...
    setRenameValue((profiles[index] as { name: string }).name);
  };
  ...
  const commitRename = (index: number) => {
    ...
    const oldName = (profiles[index] as { name: string }).name;
  ...
  const exportProfile = (index: number) => {
    const profile = profiles[index] as { name: string; mods: string[] } | undefined;
    if (!profile) return;
  ...
  const importZip = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (projectIds.size === 0) {
        ...
        const zipProfileName = file.name.replace(/\.[^/.]+$/, '');
        const profile = {
          name: zipProfileName,
        ...
        saveProfiles([...profiles, profile]);
        addDebugLog('info', `ZIP import complete: identified ${projectIds.size} mods, saved as "${zipProfileName}"`);
        await showAlert(`Identified ${projectIds.size} mods and saved as profile "${zipProfileName}"!`);
      }
    } catch (err) {
        ...
        <div className="side-menu-content">
          ...
          <div className="profile-list">
            {profiles.length === 0 ? (
              ...
              (profiles as Array<{ name: string; mods: string[]; date: string }>).map((p, i) => (
